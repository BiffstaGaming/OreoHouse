...
- **Chat windows** are real OS sub-windows, one per conversation, spawned by the main window via the Tauri WebviewWindow API. Native Windows chrome (title bar with close / minimize / maximize, OS-snap, edge-resize); drag onto any monitor; alt-tab between them like any other app. The main window owns the WebSocket; chat windows hydrate via Tauri events (see `client/src/lib/chatBridge.ts`). Each conversation's window position and size is remembered in `localStorage` (`oreohouse-chat-geom-{conv_id}`), so reopening a chat brings it back to where you last had it. Closing the [X] just closes the window — the conversation stays in the contact list, click it again to reopen.
- **System tray** is wired by the Tauri shell: left-click toggles the main window, right-click opens a Show / Quit menu, and the **main window's** [X] button hides to tray rather than exiting. Quit is only reachable via the tray menu, so the app keeps receiving WebSocket events in the background. Chat sub-windows' [X] buttons close normally (only the main window is intercepted for the close-to-tray gesture).
