...
- [x] **Phase 7** — Old-school feel: custom status (online / away / busy + free-text line, persisted server-side), typing indicators with 2 s sender throttle + 5 s receiver expiry, nudges with a 3 s cooldown that shake the recipient window for ~700 ms, synthesised Web Audio message blip + nudge rumble with a per-machine mute toggle, and an MSN-palette CSS pass (blue gradient title bars, beveled buttons, radial-gradient status dots).
- [x] **Phase 8** — Admin panel: `users.is_admin` column with first-CLI-user bootstrap and `oreohouse user promote/demote`, REST endpoints `GET / POST /api/admin/users` + `PUT /api/admin/users/{id}/password` behind a `requireAdmin` middleware, and a tiny embedded web UI at `/admin/` (vanilla HTML + ES modules, no build step — see `docs/decisions/0002-admin-ui-vanilla-html.md`).
